...
int quieroEntrar = 0, miID, *colaNodos, pendientes = 0;
int nUsuarios;
...
int main(int argc, char* argv[]) {
    ...
    //Si no introduce los usuarios salimos
    if (argc < 2){
        printf("Uso: %s <clave> <Nº usuarios>\n", argv[0]);
        exit(-1);
    }

    nUsuarios = atoi(argv[2]);
    msg mensajeOut;
    pthread_t idHilo;
    int error, idNodos[nUsuarios - 1];

    // Inicialización dinámica de colaNodos
    colaNodos = (int*)malloc(sizeof(int) * (nUsuarios - 1));
    if (colaNodos == NULL) {
        printf("Error al asignar memoria para colaNodos.\n");
        return -1;
    }
